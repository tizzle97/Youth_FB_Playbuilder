...
empty. Generate ~20 flag plays (5v5/7v7: double slants, trips z-curl,
mesh, center screen, etc.) and ~15 11v11 basics (Power, Iso, slant-flat,
four verts) as `canvas_data` JSON (version 3: paths/playerIcons/zones,
normalized 0–1 coords) under an official account (`system@playbook.pro`
exists from the original seed; decide whether to use it or a fresh
"Playbuilder Pro" account with a proper username/avatar). Ship as
`is_public=false` drafts → Jeremy reviews each in the designer → flips
public. Thumbnails must be generated by driving the real designer
headlessly (load JSON → `exportImage()` → save) so seeded plays render
identically to user-made ones. Only original renditions of classic
public-domain concepts — never trace diagrams from commercial playbook
products. Tag every play with game format + situation + difficulty in
`metadata` so library browsing feels curated.
